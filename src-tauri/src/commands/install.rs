use crate::{get_cached_password, install, password, set_cached_password};

#[tauri::command]
pub async fn get_install_command(
    tool_name: String,
) -> Result<install::InstallCommandResult, String> {
...
) -> Result<Vec<install::InstallResult>, String> {
    let pwd = password
        .or_else(get_cached_password)
        .ok_or("Senha não fornecida. Use set_password primeiro.")?;
