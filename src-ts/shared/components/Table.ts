...
export function createTable<T extends Record<string, unknown>>(options: TableOptions<T>): HTMLElement {
  const {
    columns,
    data,
    sortable = true,
  ...
  columns.forEach(col => {
    ...
    if (sortable && col.sortable) {
      ...
      th.addEventListener('click', () => handleSort(col.key as string));
    }
  ...
  function updateSortIndicators(): void {
    ...
    if (sortField) {
      ...
      if (colIndex >= 0) {
        const th = headerRow.children[colIndex] as HTMLElement;
