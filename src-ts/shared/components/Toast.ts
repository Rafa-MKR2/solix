...
const toasts = new Map<number, ToastInstance>();
...
export function updateToast(id: number, options: Partial<ToastOptions>): void {
  ...
  if (options.type) {
    const oldType = Array.from(element.classList).find(c => c.startsWith('toast-'))?.replace('toast-', '') as ToastType;
    if (oldType) {
    ...
    if (icon) icon.textContent = TOAST_ICONS[options.type];
  }
  ...
  if (options.duration && !options.persistent) {
    ...
    if (existing) toasts.set(id, { ...existing, timeoutId });
  }
}

export function showSuccess(title: string, message?: string, duration?: number): number {
  return showToast({ type: 'success', title, message, duration });
}

export function showError(title: string, message?: string, duration?: number): number {
  return showToast({ type: 'error', title, message, duration });
}

export function showWarning(title: string, message?: string, duration?: number): number {
  return showToast({ type: 'warning', title, message, duration });
}

export function showInfo(title: string, message?: string, duration?: number): number {
  return showToast({ type: 'info', title, message, duration });
}

export function showLoading(title: string, message?: string): number {
