...
import type { DiskInfo, DiskUsageItem } from '../../shared/types/index.js';
...
function parseSizeGB(sizeStr: string): number {
  ...
  if (s.includes('TB') || s.includes('TiB')) return parseFloat(s) * 1024;
  if (s.includes('GB') || s.includes('GiB')) return parseFloat(s);
  if (s.includes('MB') || s.includes('MiB')) return parseFloat(s) / 1024;
  if (s.includes('KB') || s.includes('KiB')) return parseFloat(s) / (1024 * 1024);
  return 0;
}
...
// Module-level event listeners for disk analysis overlay
