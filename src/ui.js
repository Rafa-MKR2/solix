...
export async function handleShowSmartInfo(device) {
    ...
    try {
        ...
        if (healthSection) {
            ...
            if (modelEl)
                modelEl.textContent = info.device_model || '—';
            if (tempEl)
                tempEl.textContent = info.temperature || '—';
            if (hoursEl)
                hoursEl.textContent = info.power_on_hours || '—';
        ...
        if (errorSection) {
            ...
            const msgEl = document.getElementById('smart-error-msg');
            if (msgEl)
                msgEl.innerHTML = `❌ Erro ao consultar S.M.A.R.T.: ${escapeHtml(e + '')}<br><br>Verifique se o pacote smartmontools está instalado.`;
        }
    }
}
