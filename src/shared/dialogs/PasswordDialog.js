...
export class PasswordDialog {
    ...
    bindEvents(isAppUpdate) {
        if (!this.dialog)
            return;
        const footer = this.dialog['dialog']?.querySelector('.dialog-footer');
        if (!footer)
            return;
        this.confirmBtn = footer.querySelector('[data-action="confirm"]');
        const cancelBtn = footer.querySelector('[data-action="cancel"]');
        if (!isAppUpdate) {
            this.input = this.dialog['dialog']?.querySelector('#dialog-password-input');
            this.errorEl = this.dialog['dialog']?.querySelector('#dialog-password-error');
