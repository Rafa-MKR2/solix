export function createProgressBar(options) {
    const { value, max = 100, label, showPercent = true, animated = true, striped = true, color = 'primary', size = 'md', className = '', } = options;
...
export function updateProgressBar(container, value, max = 100) {
    const fill = container.querySelector('.progress-bar-fill');
    const percentEl = container.querySelector('.progress-bar-percent');
    if (!fill)
        return;
    const clampedValue = Math.max(0, Math.min(max, value));
    const percentage = (clampedValue / max) * 100;
