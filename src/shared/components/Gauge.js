...
export function createGauge(options) {
    const { value, max = 100, min = 0, label, unit = '%', size = 120, strokeWidth = 10, colors = DEFAULT_COLORS, thresholds = DEFAULT_THRESHOLDS, showValue = true, animate = true, } = options;
    const radius = (size - strokeWidth) / 2;
    const circumference = 2 * Math.PI * radius;
    const clampedValue = Math.max(min, Math.min(max, value));
    const percentage = (clampedValue - min) / (max - min);
    let color = colors.low;
    if (percentage > thresholds.high / 100)
        color = colors.critical;
    else if (percentage > thresholds.medium / 100)
        color = colors.high;
    else if (percentage > thresholds.low / 100)
        color = colors.medium;
...
export function updateGauge(container, value, options = {}) {
    const progressCircle = container.querySelector('.gauge-fill');
